#!/usr/bin/env python3
"""
Export a flat manifest of all files in the library (RFA only - excludes ZIP files)
"""
...
def export_manifest(root: Path, output_path: Path) -> None:
    """Export manifest of all files to JSONL or CSV format (excluding ZIP files)."""
    products = []
    ...
    for json_file in root.rglob("product.json"):
        try:
            ...
            # Process each file (excluding ZIP files)
            for file_data in product_data.get("files", []):
                # Skip ZIP files (RFA only policy)
                if file_data.get("ext") == ".zip":
                    continue
    ...
    print(f"Exported {len(products)} file records to {output_path} (ZIP files excluded)")
...
def main():
    parser = argparse.ArgumentParser(description='Export flat manifest of all files in library (RFA only)')
    parser.add_argument('--root', default='./library', help='Library root directory')
    parser.add_argument('--out', default='manifest_rfa_only.jsonl', help='Output file (CSV if .csv extension)')
    ...
    print(f"Exporting manifest from {root} to {output_path} (RFA only policy)")
